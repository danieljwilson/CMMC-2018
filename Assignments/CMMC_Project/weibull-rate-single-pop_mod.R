...
hist(mid_mat[3,], breaks = 30)

# Step 2, Analysis
I = length(mid_mat[,1])   # define number of participants
...
psi[1,]=apply(mid_mat,1,min, na.rm=TRUE) -.05 #for ea. participant, set psi as min(RT)-.05 sec
...
for (i in 1:I)
{
  # set dat value since rows are differeing lengths (normally dat=y[i,])
  fix_vals = mid_mat[i,]
  fix_vals = fix_vals[!is.na(fix_vals)]
  J = length(fix_vals)

  psi[m,i] = arms(psi[m-1,i],ld.psi,ind.psi,1,dat=fix_vals,
              lambda=lambda[m-1,i],beta=beta[m-1,i])
	
	lambda[m,i] = rgamma(1,xi1[m-1]+J,xi2[m-1]+sum((fix_vals-psi[m,i])^beta[m-1,i]))
	
	beta[m,i] = arms(beta[m-1,i],ld.beta,ind.beta,1,dat=fix_vals,
...
# Plot subject with Rts & Estimated weibull
i = 2
y_ij ~ Weibull(est.psi[i],est.lambda[i],est.beta[i])

y_test = dweibull(rt-est.psi[i],scale=est.lambda[i],shape=est.beta[i]) #,log=TRUE



hist(mid_mat[i,], probability = TRUE, breaks=40)
lines(0:6000, dweibull((0:6000)-est.psi[i], shape=est.lambda[i], scale=est.beta[i]), col=3, lwd=2)
