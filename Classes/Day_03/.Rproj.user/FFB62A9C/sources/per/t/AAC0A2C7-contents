...
t2tsd  <- c(0.,0.01)   # trial to trial standard deviation (starting point, drift rate)
...
for (i in c(1:nreps)) { 
  ...
  evidence[i,] <- cumsum(c(sp,rnorm(nsamples,dr,sdrw)))  # instead of adding zero add random starting point (sp)
...
hist(botrt,col="gray",
 ...
 savePlot(filename = paste("rwRTt2t",as.character(drift),
                           as.character(t2tsd[1]),as.character(t2tsd[2]),
                           ".pdf",sep=""),
