...
myinits <-	list(
  list(alpha = rep(0.5, ns), beta = rep(0.1,ns)))        # ? Why a list inside of a list?

#myinits <-	list(
#  list(alpha = 0.5, beta = 0.1))        # ? Why a list inside of a list?
...
samples <- jags(data, inits=myinits, parameters,
                model.file ="forgetting_model_individual.txt",
...
par(mar=c(2,2,1,0))
plot(alpha[i],beta[i], xlab="", ylab="", xlim=c(0,1),ylim=c(0,1),axes=F)
box(lty=1)

par(mar=c(2,2,1,4))
plot(d.beta[i]$y, d.beta[i]$x, ylim=range(c(0,1)), xlim=rev(range(d.beta$y)), type='l', axes=F, xlab="", ylab="")
axis(4, at=c(0,1))
mtext(expression(beta[i]), side=4,line=1, cex=1.3)
box(lty=1)

par(mar=c(6,2,0,0))
plot(density(alpha[i]),zero.line=F, main="", ylab="", xlab="", cex.lab=1.3,xlim=c(0,1), axes=F)
axis(1,at=c(0,1))
