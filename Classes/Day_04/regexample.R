...
getregpred <- function(parms,data) {
  getregpred <- parms["b0"] + parms["b1"]*data[ ,2]     # this is where you would pass in the function that is predicting your values
                                                        # this could be ddm etc...whatever your model is
  
  #wait with drawing a graph until key is pressed
  # this code is just for the animation
  par(ask=TRUE)
...
nDataPts  <- 20
rho       <- .8     # correlation between the two variables
intercept <- .0
...
startParms <- c(-1., .2)              # order = (slope, intercept)
names(startParms) <- c("b1", "b0")    # name columns
#obtain parameter estimates
xout <- optim(startParms, rmsd, data1=data)     # this runs SIMPLEX

# optim takes three arguments
# 1. starting values
# 2. name of descrepancy function (in this case rmsd)
# 3. which variable gets passed to discrepancy function
